...
class ChapterRepository
{
    ...
    {
        $chapter = $this->chapter->withDeleted()->find($id);
        return $chapter;
    }
    ...
    public function create(Request $request)
    {
        ...
        try {
            $request->merge([
                ...
                'order' => $request->order ? $request->order : ($this->chapter->count() + 1),
            ]);
            $chapter = $this->chapter->create($request->except('title'));
    ...
    {
        $query = $this->chapter->with(['translations'])->where(function ($query) use ($request) {
