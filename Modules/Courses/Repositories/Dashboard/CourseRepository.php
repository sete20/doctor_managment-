...
class CourseRepository
{
    ...
    {
        $courses = $this->course->orderBy($order, $sort)->count();
        return $courses;
    }

    public function mainCategories($order = 'id', $sort = 'desc')
    {
        $courses = $this->course->mainCategories()->orderBy($order, $sort)->get();
        return $courses;
    }
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $course = $this->course->create($request->except('title','description','note'));
            ...
            if ($request->file('intro_video')) {
                $course->addMediaFromRequest('intro_video')->toMediaCollection('intro_video');
            }
    ...
    {
        $query = $this->course->with(['translations'])->where(function ($query) use ($request) {
