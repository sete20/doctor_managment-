	...
	$(function() {
		...
		$(document)
			...
			.on('dnd_move.vakata.jstree', function (e, data) {
				...
				if(ins && ins._data && ins._data.dnd) {
					...
					if( (data.event.target === ins.element[0] || data.event.target === ins.get_container_ul()[0]) && ins.get_container_ul().children().length === 0) {
						...
						if(ok) {
							...
							marker.hide();
							data.helpers.find('.jstree-icon').first().removeClass('jstree-er').addClass('jstree-ok');
							return;
						}
					}
					else {
				...
				lastmv = false;
				data.helpers.find('.jstree-icon').removeClass('jstree-ok').addClass('jstree-er');
				marker.hide();
			})
			.on('dnd_scroll.vakata.jstree', function (e, data) {
				...
				lastmv = false;
				lastev = false;
				data.helpers.find('.jstree-icon').first().removeClass('jstree-ok').addClass('jstree-er');
			})
